...
function [MTX] = KELEME(TIPR,XYE,CAE)
  ...
  switch TIPE
    case 201 % elemento 2D triangular lineal
      [MTX] = KTRIEL(TIPR,XYE,CAE);
    case 202 % elemento 2D cuadrilateral bilineal
      [MTX] = KCUDEL(TIPR,XYE,CAE);
    case 203 % elemento 2D triangular cuadrático
      % pendiente
    case 302 % elemento 2D cuadrilateral bicuadrático
      % pendiente
    otherwise
      error('KELEME. Tipo incorrecto de elemento finito');
  end
...
function [MTX] = KTRIEL(TIPR,XYE,CAE)
  ...
  [DJAC,BEL] = BELEME(XYE,0,TIPE); % matriz B
